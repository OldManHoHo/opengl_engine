...
std::vector <glm::vec3> TGLPhysicsEngine::get_world_blocks(TGLActor * in_actor, TGLChunkSpawn * chunks_spawner)
{
	...
	x[0] = int(0.01+round(position.x));
	x[1] = int(0.01+round(position.x)) - 1;
	x[2] = int(0.01+round(position.x)) + 1;

	y[0] = int(0.01+round(position.y));
	y[1] = int(0.01+round(position.y)) - 1;
	y[2] = int(0.01+round(position.y)) + 1;
	y[3] = int(0.01+round(position.y)) - 2;
	y[4] = int(0.01+round(position.y)) + 2;

	z[0] = floor(position.z - (actor_bounds)->depth / 2.0);
	z[1] = floor(position.z + (actor_bounds)->depth / 2.0);
	z[2] = int(0.01+round(position.z)) + 1;
	z[0] = int(0.01+round(position.z));
	z[1] = int(0.01+round(position.z)) - 1;
	z[2] = int(0.01+round(position.z)) + 1;
