...
void Simplex_TEST()
{
	...
	for (int s = 0; s < 100; s += 3)
	{
		...
		for (int i = 0; i < simplexes.size(); ++i)
		{
			...
			{
				for (int offset = -size_val*2; offset < size_val * 2; offset += size_val)
				{
					{
						std::vector <float> points(size_val*size_val);
						std::vector <float> points2(size_val*size_val);
						float * out = simplexes[i]->get_points_2d(offset,offset,size_val);
						std::copy(out, out+size_val*size_val, points.begin());
						out = simplexes[i]->get_points_2d(offset + int(size_val/2.0), offset + int(size_val/2.0),size_val);
						std::copy(out, out+size_val*size_val, points2.begin());
						for (int j = 0; j < int(size_val/2.0); ++j)
						{
							for (int k = 0; k < int(size_val/2.0); ++k)
							{
								if (points[size_val*(k+int(size_val/2.0)) + j + int(size_val/2.0)] != points2[size_val*(k) + j])
								{
									std::cout << "Simplex Unit Test FAILED on simplex: " << i << "\n";
									std::cout << "\tValues: " << points[size_val*(k+int(size_val/2.0)) + j + int(size_val/2.0)] << ", " << points2[size_val*(k) + j] << "\n";
									assert(false);
								}
					...
					}
					{
						...
						out = simplexes[i]->get_points(offset + int(size_val/2.0), offset + int(size_val/2.0),0,size_val);
						...
						for (int j = 0; j < int(size_val/2.0); ++j)
						{
							for (int k = 0; k < int(size_val/2.0); ++k)
							{
								...
								{
									if (points[size_val*256*(k+int(size_val/2.0)) + 256*(j + int(size_val/2.0)) + m] != points2[size_val*256*(k) + 256*(j) + m])
									{
										std::cout << "Simplex 3D Unit Test FAILED on simplex: " << i << "\n";
										std::cout << "\tValues: " << points[size_val*256*(k+int(size_val/2.0)) + 256*(j + int(size_val/2.0)) + m] << ", " << points2[size_val*256*(k) + 256*(j) + m] << "\n";
										assert(false);
									}
								}
