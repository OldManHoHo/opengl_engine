...
void Base::generate_game_state(bool full)
{
	...
	*num_actors = 0;
    ...
    std::vector <block_def>& changes = ((tmc::ChunkSpawn*)chunks_spawner)->get_block_changes();
...
void Base::process_msg(std::pair<sockaddr_in, std::vector<char>>* in_pair)
{
	...
	else if ((tgl::NetMsgType)in_pair->second[0] == tgl::NetMsgType::PlayerInput)
	{
		if (clients.find(client_addr) != clients.end())
		{
...
bool Base::set_conf_value(std::string conf_var_name, std::string conf_var_value_str, bool only_print)
{
	bool found = true;
...
void Base::start_tasks()
{
	if (cpu_lighting_enabled)
	{
		...
		light_thread = new std::thread([&](tmc::ChunkSpawn * cs) { cs->recalculate_light(); }, (tmc::ChunkSpawn*)chunks_spawner);
	}
		...
		if (shadow_maps_enabled && shadow_map_counter > shadow_map_interval)
		{
			...
			//for (int i = actors.size()-1; i >=0; --i)
			{
				...
				if (actors[i]->is_chunk)
				{
					...
					int chunk_x, chunk_y;
					((tmc::ChunkSpawn*)chunks_spawner)->get_chunk_of_point(act_chunk->get_pos() + glm::vec3(1, 0, 1), chunk_x, chunk_y);
					if (!((tmc::ChunkSpawn*)chunks_spawner)->chunk_in_fov(chunk_x, chunk_y, active_camera->get_pos(), ((tgl::Player*)active_camera)->forward_vec))
					{
						continue;
					}
				}
				std::vector <tgl::Component*> components = actors[i]->get_components();
				//std::vector <tgl::Component*> components = (*actor_it)->get_components();
		...
		for (int i = 0; i < actors.size(); ++i)
		{
			...
			for (auto mesh_it = components.end() - 1; mesh_it != components.begin() - 1; --mesh_it)
			{
				...
				if ((*mesh_it)->get_draw_flag())
				{
					...
					std::vector <tgl::Texture*> textures = mesh_comp->get_textures();
					int count = 0;
...
// PHYSICS UPDATE
#ifdef _TGL_SERVER
		physics_engine.tick(time_delta, actors, (tmc::ChunkSpawn*)chunks_spawner, gravity_enabled);
#endif
