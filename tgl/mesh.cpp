...
GLfloat * calculate_normals(GLfloat * vertices, int length)
{
    ...
    for (int i = 0; i < length; i=i+9)
    {
        ...
        out_normals[i] =
            out_vec[0]; out_normals[i + 1] =
            out_vec[1]; out_normals[i + 2] =
            out_vec[2];
        out_normals[i + 3] =
            out_vec[0]; out_normals[i + 4] =
            out_vec[1]; out_normals[i + 5] =
            out_vec[2];
        out_normals[i + 6] =
            out_vec[0]; out_normals[i + 7] =
            out_vec[1]; out_normals[i + 8] =
            out_vec[2];
    }
    /*
    for (int i = 0; i < length; i+=3)
...
Mesh::Mesh(GLfloat * vertices, int in_length)
{
    ...
    glVertexAttribPointer(vertex_position_attrib,
                          3, GL_FLOAT,
                          GL_FALSE,
                          3 * sizeof(float),
                          static_cast<void*>(0));
...
Mesh::Mesh(MeshVertices const* in_vertices)
{
    ...
    glVertexAttribPointer(vertex_normal_attrib,
                          3,
                          GL_FLOAT,
                          GL_FALSE,
                          3 * sizeof(float),
...
void Mesh::add_texture(tgl::Texture * in_texture,
                       GLfloat * in_coords,
                       int coord_length)
{
    texture_objects to;
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 coord_length * sizeof(GLfloat),
                 in_coords,
                 GL_STATIC_DRAW);
    to.attrib = textures.size() + 10;
    glVertexAttribPointer(to.attrib,
                          2,
                          GL_FLOAT,
                          GL_FALSE,
                          2 * sizeof(float),
    ...
    std::string tex_name =
        ("texture" + std::to_string(textures.size() - 1)).c_str();
...
void Mesh::enable_instancing(GLfloat * instance_locations,
                             int in_instance_count,
                             int in_unused_count)
{
    int err;
    instance_flag = true;
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 (instance_count + in_unused_count) * 3 * sizeof(GLfloat),
                 instance_locations,
                 GL_DYNAMIC_DRAW);
    ...
    glVertexAttribPointer(instance_attrib,
                          3,
                          GL_FLOAT,
                          GL_FALSE,
                          3 * sizeof(float),
...
void Mesh::remove_instance(int index)
{
    ...
    GLfloat * temp_buf =
        new GLfloat[(instance_count-1) * 3 * sizeof(GLfloat) -
                    index * 3 * sizeof(GLfloat)];
    glGetBufferSubData(GL_ARRAY_BUFFER,
                       (index+1) * 3 * sizeof(GLfloat),
                       (instance_count-1) * 3 * sizeof(GLfloat) -
                            (index) * 3 * sizeof(GLfloat),
                       temp_buf);
    ...
    if (light_data_enabled)
    {
        ...
        unsigned char * temp_c =
            new unsigned char[(instance_count - 1) * sizeof(GLbyte) -
                              index * sizeof(GLbyte)];
        glGetBufferSubData(GL_ARRAY_BUFFER,
                           (index + 1) * sizeof(GLbyte),
                           (instance_count - 1) * sizeof(GLbyte) -
                                (index) * sizeof(GLbyte),
                           temp_c);
        glBufferSubData(GL_ARRAY_BUFFER,
                        (index) * sizeof(GLbyte),
...
int Mesh::add_instance(glm::vec3 loc)
{
    ...
    else
    {
        ...
        instance_count += 1;
        glBufferData(GL_ARRAY_BUFFER,
                     (instance_count + 50) * 3 * sizeof(GLfloat),
                     &local_vbo_mem[0],
                     GL_DYNAMIC_DRAW);
        refresh_instances();
        ...
        if (light_data_enabled)
        {
            ...
            buffer_size = instance_count + 50;
            glBufferData(GL_ARRAY_BUFFER,
                         (instance_count - 1 + 50) * sizeof(GLbyte),
                         &local_light_mem[0],
                         GL_DYNAMIC_DRAW);
        }
...
void Mesh::enable_light_data(int unused)
{
    if (0)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER,
                     (instance_count + unused) * sizeof(GLbyte),
                     &zeroes[0],
                     GL_DYNAMIC_DRAW);

        light_attrib = new_attrib();
        glVertexAttribPointer(light_attrib,
                              1,
                              GL_UNSIGNED_BYTE,
                              GL_FALSE,
                              sizeof(GLbyte),
    ...
    else
    {
        ...
        glBufferData(GL_ARRAY_BUFFER,
                     (instance_count + unused) * 3 * sizeof(GLfloat),
                     &zeroes[0],
                     GL_DYNAMIC_DRAW);
...
void Mesh::refresh_light_data(std::vector <unsigned char>& in_data)
{
    if (local_vbo_mem.size())
    {
        ...
        glBufferData(GL_ARRAY_BUFFER,
                     (local_vbo_mem.size()/3) * sizeof(GLbyte),
                     &in_data[0],
                     GL_DYNAMIC_DRAW);
        while ((err = glGetError()) != GL_NO_ERROR)
        {
...
void Mesh::refresh_light_data_vec(std::vector <GLfloat>& in_data)
{
    if (local_vbo_mem.size())
    {
        ...
        glBufferData(GL_ARRAY_BUFFER,
                     (local_vbo_mem.size()) * sizeof(GLfloat),
                     &in_data[0],
                     GL_DYNAMIC_DRAW);
        while ((err = glGetError()) != GL_NO_ERROR)
        {
