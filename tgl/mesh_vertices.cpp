...
void MeshVertices::calculate_normals(std::vector<GLfloat> const &vertices)
{
    ...
    for (int i = 0; i < length; i = i + 9)
    {
        ...
        out_normals[i] = out_vec[0];
        out_normals[i + 1] = out_vec[1];
        out_normals[i + 2] = out_vec[2];
        out_normals[i + 3] = out_vec[0];
        out_normals[i + 4] = out_vec[1];
        out_normals[i + 5] = out_vec[2];
        out_normals[i + 6] = out_vec[0];
        out_normals[i + 7] = out_vec[1];
        out_normals[i + 8] = out_vec[2];
    }
    for (int i = 0; i < length; i += 3)
    {
        // out_normals[i] = 0.0;
