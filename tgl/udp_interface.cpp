...
{
    receive_thread = new std::thread([&] (UDPInterface * player) { player->receive_loop(); }, this);	
...
void UDPInterface::pop_msg(std::pair <sockaddr_in,std::vector<char>>*& out_pair)
{
    buffer_queue.pop_back(out_pair);
}

void UDPInterface::return_msg(std::pair <sockaddr_in,std::vector<char>>*& in_pair)
{
